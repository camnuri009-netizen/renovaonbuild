...
function loadServices(){
  ...
  serviceList.innerHTML=services.map((x,i)=>`
    <div class="serviceEditBox">
      ...
      ${x.mainImage ? `<div class="serviceEditImages"><img src="${x.mainImage}"></div>` : ""}
      ...
      <div class="serviceEditImages">
        ${(x.detailImages||[]).map((img,j)=>`<div class="imgBox"><img src="${img}"><button onclick="removeServiceDetailImage(${i},${j})">X</button></div>`).join("")}
      </div>
      <input type="file" accept="image/*" multiple onchange="addServiceDetailImages(${i},this)">

      <button onclick="moveService(${i},-1)">위로</button>
      <button onclick="moveService(${i},1)">아래로</button>
...
function addService(){
  ...
  s.push({title:"새 서비스",desc:"서비스 설명",detailDesc:"상세 설명",mainImage:"",detailImages:[]});
